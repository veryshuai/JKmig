// This header file contains several functions which I use through the sequence of c++ programs for use with my migration growth project.
...
namespace constants {
  ...
  const double  FALP            = .02;    //foreign meeting hazard
  const int     GRIDSIZE        = 1000;   //number of grid points
  const int     GRIDMAX         = 200;    //maximum grid value
  const int     PERIODS         = 20;     //number of periods
  const double  PERIOD_LENGTH   = .5;     //length of a period (say a unit is a year)
